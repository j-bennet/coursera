...
(* helper function to shift things *)      
fun shift_by (dx, dy, e) =
  case e of
    NoPoints => e
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      NONE => raise BadProgram("var not found: " ^ s)
       ...
       | Shift(dx, dy, e1) => shift_by(dx, dy, eval_prog(e1,env))
(* CHANGE: Add a case for Shift expressions *)

fun preprocess_line_segment (seg) =
	case seg of LineSegment (x1, y1, x2, y2) => 
		if (real_close(x1, x2) andalso real_close(y1, y2))
    then Point(x1, y1)
    else if ((real_not_close(x1, x2) andalso (x2 < x1)) orelse (real_close(x1, x2) andalso (y2 < y1)))
...
fun preprocess_prog (e) =
    ...
    | LineSegment _ => preprocess_line_segment(e)
