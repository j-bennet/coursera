...
x1 = LineSegment.new(ONE, TWO, ONE, ONE)
x2 = x1.preprocess_prog
if not ((x2.is_a? LineSegment) and (x2.x1 == ONE and x2.y1 == ONE and x2.x2 == ONE and x2.y2 == TWO))
	puts "LineSegment.new(1.0,2.0,1.0,1.0) should preprocess to LineSegment.new(1.0,1.0,1.0,2.0), actual: #{x2}"
end
...
l3 = Let.new("x", Let.new("x", NoPoints.new, Point.new(ZERO, ZERO)), Var.new("x"))
l3 = l3.preprocess_prog.eval_prog([["x", NoPoints.new], ["x", Point.new(0.0,0.0)]])
if not (l3.x == ZERO and l3.y == ZERO)
	puts 'Let.new("x",Let.new("x",NoPoints.new,Point.new(0.0,0.0)),Var.new("x")) should evaluate to Point.new(0.0,0.0) given env==[["x", NoPoints.new], ["x", Point.new(0.0,0.0)]], actual: ' + l3.to_s
end

l4 = Let.new("x",NoPoints.new,Let.new("x",Point.new(0.0,0.0),Var.new("x")))
l4 = l4.preprocess_prog.eval_prog([["x", NoPoints.new], ["x", Point.new(0.0,0.0)]])
if not (l4.x == ZERO and l4.y == ZERO)
	puts 'Let.new("x",NoPoints.new,Let.new("x",Point.new(0.0,0.0),Var.new("x"))) should evaluate to Point.new(0.0,0.0) given env==[["x", NoPoints.new], ["x", Point.new(0.0,0.0)]], actual: ' + l4.to_s
end

v1 = Var.new("x")
v1 = v1.preprocess_prog.eval_prog([["x", NoPoints.new], ["x", Point.new(0.0,0.0)]])
if not (v1.is_a? NoPoints)
	puts 'Var.new("x") should evaluate to NoPoints.new given env==[["x", NoPoints.new], ["x", Point.new(0.0,0.0)]], actual: ' + v1.to_s
end

v1 = Var.new("x")
v1 = v1.preprocess_prog.eval_prog([["x", NoPoints.new], ["y", Point.new(0.0,0.0)], ["x", Point.new(3.0,2.0)]])
if not (v1.is_a? NoPoints)
	puts 'Var.new("x") should evaluate to NoPoints.new given env==[["x", NoPoints.new], ["y", Point.new(0.0,0.0)], ["x", Point.new(3.0,2.0)]], actual: ' + v1.to_s
end
