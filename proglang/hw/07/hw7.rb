...
class LineSegment < GeometryValue
  ...
  def to_s
    "LineSegment x1: #{@x1} y1: #{@y1} x2: #{@x2} y2: #{@y2}"
  end
  def eval_prog env 
    self
  end
  def preprocess_prog
    if real_close_point(x1, y1, x2, y2)
      Point.new(x1, y1)
    elsif (not_real_close(x1, x2) and (x2 < x1)) or (real_close(x1, x2) and (y2 < y1))
  ...
  def intersectWithSegmentAsLineResult seg
      ...
      aXstart = aYstart = aXend = aYend = nil
      bXstart = bYstart = bXend = bYend = nil
      if y1 < seg.y1
...
class Let < GeometryExpression
  ...
  def eval_prog env
    ...
    newenv = newenv.push([@s, newv]) if (!added)
    @e2.eval_prog(newenv)
  end
  def preprocess_prog
